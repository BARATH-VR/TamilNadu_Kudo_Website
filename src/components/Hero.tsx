...
export const Hero: React.FC<HeroProps> = ({ setCurrentView }) => {
  ...
  return (
    <div className="relative bg-zinc-950 text-white pt-14 pb-20 overflow-hidden border-b border-amber-500/20 w-full">
      
      {/* Brilliant Pure Gold Centered Radial Aura Glow */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] bg-amber-500/12 rounded-full blur-[160px] pointer-events-none"></div>

      <div className="max-w-[1920px] mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
        <div className="max-w-4xl lg:max-w-5xl mx-auto text-center space-y-8 flex flex-col items-center">
          
          {/* Badge pill */}
          <div className="inline-flex items-center space-x-2 bg-zinc-900/90 border border-amber-500/40 px-4.5 py-1.5 rounded-full text-xs font-bold text-amber-300 shadow-lg backdrop-blur-md">
            <ShieldCheck className="w-4 h-4 text-amber-400 shrink-0" />
            <span>{t.heroBadge}</span>
          </div>

          {/* Meaningful Main Headline */}
          <h1 className="text-3xl sm:text-5xl lg:text-6xl xl:text-7xl font-black text-white leading-[1.1] tracking-tight max-w-4xl">
            {language === 'en' ? (
              <>
          ...
          {/* 3 STAT COUNTER CARDS (Centered Layout) */}
          <div className="grid grid-cols-3 gap-3 sm:gap-6 w-full max-w-2xl pt-4">
          ...
          <div className="pt-6 border-t border-zinc-800/80 flex flex-wrap justify-center items-center gap-6 text-xs sm:text-sm text-zinc-300 w-full">
